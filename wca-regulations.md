...
- 9f) 每一輪成績的計算方式如下：
    ...
    - 9f4) 如果某一次還原未完成或是該次還原被判定無效，則該次成績記錄為DNF（未完成）。
        - 9f4a) 如果很明顯一位參賽者違反規則到要被取消該次還原成績，裁判應該要立即停止那次還原。如果裁判不確定或是可能會造成不小的爭議（例如：額外還原機會會造成比賽延遲），裁判應該要諮詢WCA代表。    - 9f5) 如果參賽者放棄比賽中某次還原機會，則該次成績記錄為DNS（未開始）。
