...
- 5b) 當方塊故障發生後，參賽者可以選擇修復方塊並繼續還原，或是選擇停止還原。
    ...
    - 5b5) 若在解方塊階段結束後，方塊的一些零件掉出或是沒有完全歸位，則依照下面規則判定：
        ...
        - 5b5f) 如果一個零件半分離或是沒有完全歸位，考慮方塊的現有狀態，這個零件的最終位置被視為一般正常機構運作下最在方塊中接近的位置。如果最終的位置會讓方塊解完成，這個零件不被視為被方塊故障影響。
    - 5b6) 定義：一個零件是一個或是多個連結在一起的部件群，且在一般操作下跟其他零件沒有相對的移動關係。（例如：一個3x3x3方塊的中心蓋、一個金字塔Pyraminx的頂部件、少一個角塊蓋的3x3x3方塊角塊，一個3x3x3方塊的角塊蓋）。
