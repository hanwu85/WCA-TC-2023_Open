...
- 3h) 不允許對方塊進行違反基本概念的改造，若修改後的方塊相較於修改前的方塊，不會給參賽者在比賽中提供更多資訊（例如：方塊零件的方向、直接分辨方塊的零件），此方塊便允許使用。
    ...
    - 3h5) 鬆動的按鈕（也就是說不用按壓就能靠重力改變狀態的按鈕）不被視為合理的使用磨損，所以這樣的狀態是禁止被允許的。
...
- 9f) 每一輪成績的計算方式如下：
    ...
    - 9f2) 所有在10分鐘以上的計時成績和平均成績，四捨五入到秒的個位數（例如：X.49記為 X，X.5記為 X+1）。
...
- 12g) 魔錶Clock的轉動代號：
    ...
    - 12g3) 轉動朝上狀態的按鈕旁邊的轉鈕，然後把全部按鈕往下按：X+（順時鐘轉*齒）、X-（逆時針轉*齒）。
...
- A1) 以速解為目標的比賽還原必須要遵守以下流程：
    - A1a) 主辦團隊必須對每一輪做出至少一種時間限制（各次還原時間限制和/或總時間限制）。
        - A1a1) 一般默認的單次還原時間限制是10分鐘，但主辦團隊可以公告較久或較短的時間限制。某一輪中的每一次還原時間限制都要相同。
        - A1a2) 最少步數解和3x3x3多顆盲解以外的比賽項目，主辦團隊可以採用總時間限制，總時間限制必須適用於一輪中所有的還原（例如：3次還原的時間加起來須低於20分鐘），或是不同比賽項目所有還原的時間總和。在這情況下，某一次還原的時間限制就是設定的還原時間限制或是總時間限制減掉已用掉的還原時間總和，之中較短的時間。(參見[規則 A1a5](regulations:regulation:A1a5))。若某一輪比賽有總時間限制，必須只有一種總時間限制。
...
- A4) 開始解方塊：
    ...
    - A4e) 開始階段的時間處罰是採累積制的。
- A5) 解方塊中：
    - A5a) 在觀察或是解方塊當中，參賽者禁止與裁判和WCA代表以外的人交談。處罰：判定該次還原未完成（DNF）。例外：如果參賽者沒有因與裁判和WCA代表以外的人交談而得利，成績可以被保留，由WCA代表決定。
...
- A6) 結束：
    ...
    - A6b) 參賽者有責任以正確的方式停止Stackmat計時器。
        - A6b1) 如果計時器提早結束，且顯示器顯示低於 0.06秒，則可以用額外的還原機會來取代。如果WCA代表認為計時器是被故意停止的，參賽者失去額外的還原機會。
        - A6b2) 如果計時器提早結束，且顯示器顯示 0.06秒以上，則該次還原為DNF。例外：如果參賽者能夠證實計時器故障，WCA代表可以給予額外的還原機會。
